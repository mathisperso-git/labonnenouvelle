  ...
  function startCountdown() {
    const interval = setInterval(() => {
      ...
      if (count <= 0) {
        ...
        // Affiche "C’est une fille !" juste avant les feux d’artifice
        setTimeout(() => {
          ...
          // Montre le texte
          girlText.classList.remove("hidden");
          girlText.classList.add("show");
  ...
  function startFireworks() {
    ...
    class Particle {
      ...
      update(){
        ...
        if(this.trail.length>(isMobile?2:5)) this.trail.shift();
        this.vx*=FRICTION;
        this.vy*=FRICTION;
        this.vy+=GRAVITY;
        this.x+=this.vx;
    ...
    function loop(timestamp){
      ...
      if(timestamp-loop.lastTime<(isMobile?33:16)){
